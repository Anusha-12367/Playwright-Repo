...
- Name: InventoryDemoblaze.spec.js >> Add a product to cart
- Location: tests\InventoryDemoblaze.spec.js:4:1
...
```ts
  ...
  2  | import { LoginDemoblaze } from '../Pages/LoginDemoblaze'
  3  | import { InventoryPage } from '../Pages/DemoblazeInventory'
  4  | test('Add a product to cart',async({page})=>{
  5  |     const l=new LoginDemoblaze(page)
  6  |     const i=new InventoryPage(page)
  7  |     await l.goto()
> 8  |     await l.login(user,pass)
     |                   ^ ReferenceError: user is not defined
  9  |     await i.add()
  10 | })
```