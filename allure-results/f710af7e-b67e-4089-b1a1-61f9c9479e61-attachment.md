...
- Name: SignupTest.spec.js >> Signup with empty fields
- Location: tests\SignupTest.spec.js:13:1

# Error details

```
TypeError: signpage.fill is not a function
```
...
```ts
  ...
  3  | import validLogindata from '../Utils/Testdataproject.json' with {type:'json'};  
  4  | 
  5  | test('Signup in home screen',async({page})=>{
  6  |      const signpage=new SignupPage(page)
  7  |      await signpage.goto()
  8  |     
  9  |         const user=validLogindata.username
  10 |         const pass=validLogindata.password
  11 |         await signpage.signup(user,pass)
  12 | })
  13 | test('Signup with empty fields',async({page})=>{
  14 |     const signpage=new SignupPage(page)
  15 |     await signpage.goto()
> 16 |     await signpage.fill('anusha nimish','anushanimish')
     |                    ^ TypeError: signpage.fill is not a function
  17 |     await signpage.closeButton()
  18 | })
  19 | 
```