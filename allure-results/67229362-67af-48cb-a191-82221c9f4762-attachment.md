...
- Name: LogDemoblaze.spec.js >> Invalid username and invalid password
- Location: tests\LogDemoblaze.spec.js:13:1
...
```ts
  ...
  14 |     return this
  15 | }
  16 | async login(user,pass){  
  17 |     await this.loginbut.click()      
> 18 |     await this.userName.fill(user)
     |                         ^ Error: locator.fill: value: expected string, got undefined
  19 |     await this.password.fill(pass)
  20 |     await this.loginb.click() 
  21 |    // return new AddCart(this.page)
  22 | }
  23 | 
  24 | // INVALID LOGIN METHOD
  25 |     async invalidLogin(user,pass){
  26 | 
  27 |         await this.login(user,pass)
  28 | 
  29 |         // Wait for alert popup
  30 |         const alertPromise = this.page.waitForEvent('dialog')
  31 | 
  32 |         const dialog = await alertPromise
  33 | 
  34 |         const message = dialog.message()
  35 | 
